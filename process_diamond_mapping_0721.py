...
def process_diamond(csv_path, contig_fasta, paired1, paired2):
    """
    Full pipeline: DIAMOND parsing → ORF finder → MAFFT → mapping → coverage.
    """
    base_dir = os.getcwd()
            ...
            try:
                ...
                if os.path.exists(orfs_file):
                    if os.path.getsize(orfs_file) > 0:
                        ...
                        print(f"[warn] {orfs_file} is empty → removing and skipping alignment")
                        os.remove(orfs_file)
                else:
                    print(f"[warn] {orfs_file} not found → skipping alignment")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Process DIAMOND output through ORF finder, MAFFT, mapping, and coverage."
    )
...
if __name__ == "__main__":
    ...
    process_diamond(
        args.csv_file,
        args.contig_file,
        paired1=args.paired1,
        paired2=args.paired2
    )
